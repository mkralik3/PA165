...
public class CarShopStorageServiceImpl implements CarShopStorageService {
	...
	public void sellCar(Car car) throws RequestedCarNotFoundException {
		...
		Car findedCar = carsOfSameType.stream().filter(specificCar -> specificCar.equals(car)).findFirst().get();
		if(findedCar==null){
			throw new RequestedCarNotFoundException("Car is not in the db");
		}else{
			CarShopStorage.getInstancce().getCars().get(car.getType()).remove(findedCar);
		}
	...
	public void addCarToStorage(Car car) {
		if(car.getPrice()<=0){
			throw new IllegalArgumentException("Price must be positive number but it is:" + car.getPrice());
		}
